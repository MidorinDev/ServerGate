...
public class Utils {
    ...
    public static void debug(Object msg) {
        if (true) {
            Utils.log("§8[§cServerGate§8] §cDebug: §7" + msg.toString());
        }
        debugToFile(msg);
    }
    ...
    private static void debugToFile(Object msg) {
        File debugFile = new File(ServerGatePlugins.getPlugin().getDataFolder(), "logs/latest.log");
        if (!debugFile.exists()) {
    ...
    public static void log(String message, int severity) {
        switch (severity) {
            case 1:
                ServerGatePlugins.getPlugin().getLogger().warning(message);
                break;
            case 2:
                ServerGatePlugins.getPlugin().getLogger().severe("! " + message);
                break;
            case 0:
            default:
                ServerGatePlugins.getPlugin().getLogger().info(message);
                break;
        }
    }
    ...
    public static void logToFile(String message) {
        ...
        try {
            File saveTo = new File(ServerGatePlugins.getPlugin().getDataFolder(), "debug.log");
            if (!saveTo.exists()) {
